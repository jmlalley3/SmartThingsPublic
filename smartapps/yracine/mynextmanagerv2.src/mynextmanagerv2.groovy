...
//	app.updateSetting(settingKey,5)  
	...
	if (!atomicState.accessToken) {
		...
		if ((!atomicState?.nest_user_id) && (!atomicState?.google_issue_token_url)) {
			traceEvent(settings.logFilter,"authPage>error: the smartapps's nest_user_id or google_issue_token_url appSettings variables are not populated, please read the README at the github", true, GLOBAL_LOG_ERROR, true)
	...
	if (!atomicState?.access_token) {
		...
		return dynamicPage(name: "auth", title: "Login", nextPage:null, uninstall:uninstallAllowed, submitOnChange: true) {
			section(){
				paragraph "Be sure to have populated the Nest info (user_id & access_token) or the Google info (cookie & issue_url) in appSettings first as indicated in the README file at my github"
		...
		return dynamicPage(name: "auth", title: "Log In", nextPage:"structureList", uninstall:uninstallAllowed,submitOnChange: true) {
			section(){
				paragraph "To be connected with Google/Nest, you need to have copied your login info from Nest or Google in the app settings of the smartapp at this point. If so, then just tap the upper right Next to continue the setup of your Nest devices."
...
boolean refreshParentTokens() {
	...
	if ((atomicState?.google_issue_token_url) && (isTokenExpired())) {
		if (login()) {
...
private def getAllCameras(useCache=true, cache_timeout=2) {
	...
	cameras.each {
		...
		try {
			...
			if ((e?.response?.status == TOKEN_EXPIRED) && (atomicState?.google_issue_token_url)) {
		...
		cameraData[uuid]getB << ['last_event_start_time':object?.last_event_start_time]
...
//				traceEvent(settings.logFilter, "getStructures>resp.data.buckets=${resp?.data?.updated_buckets[0]}",detailedNotif, GLOBAL_LOG_TRACE)
				if (resp?.status == NEST_SUCCESS) {
					...
					if ((resp?.status == TOKEN_EXPIRED)  && (atomicState?.google_issue_token_url)) {
					//refresh the auth token
			...
			if ((e?.response?.status == TOKEN_EXPIRED) && (atomicState?.google_issue_token_url)) { // if authenticated, then there is a problem. 
			...
			else if ((atomicState?.access_token) && (atomicState?.google_issue_token_url)) {            
				atomicState?.lastHttpStatus=e?.response?.status
...
private def get_AppSettingsValue(variable) {
	return (appSettings?."$variable" ? appSettings?."$variable".toString().trim(): "")
}
